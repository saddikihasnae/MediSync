<x-app-layout>
    ...
    <div x-data="{ activeTab: '{{ session('active_tab') ?? 'profile' }}' }" class="min-h-screen">
        <!-- Header -->
        <div class="mb-10">
            <h1 class="text-4xl font-black text-slate-800 tracking-tighter">{{ __('messages.settings') }}</h1>
            <p class="text-slate-400 font-bold mt-2 uppercase tracking-widest text-xs">{{ __('messages.manage_clinic') }}</p>
        </div>
        ...
        <div class="mb-10 flex gap-2 p-1 bg-white rounded-2xl shadow-sm border border-slate-100 w-fit">
            <button @click="activeTab = 'profile'" 
                    :class="activeTab === 'profile' ? 'bg-emerald-600 text-white shadow-lg shadow-emerald-100' : 'text-slate-400 hover:bg-slate-50 hover:text-emerald-600'" 
            ...
            <button @click="activeTab = 'clinic'" 
                    :class="activeTab === 'clinic' ? 'bg-emerald-600 text-white shadow-lg shadow-emerald-100' : 'text-slate-400 hover:bg-slate-50 hover:text-emerald-600'" 
                    class="px-6 py-2.5 rounded-xl text-sm font-black transition-all">{{ __('messages.clinic_details') }}</button>
            <button @click="activeTab = 'security'" 
                    :class="activeTab === 'security' ? 'bg-emerald-600 text-white shadow-lg shadow-emerald-100' : 'text-slate-400 hover:bg-slate-50 hover:text-emerald-600'" 
                    class="px-6 py-2.5 rounded-xl text-sm font-black transition-all">{{ __('messages.security') }}</button>
            <button @click="activeTab = 'preferences'" 
                    :class="activeTab === 'preferences' ? 'bg-emerald-600 text-white shadow-lg shadow-emerald-100' : 'text-slate-400 hover:bg-slate-50 hover:text-emerald-600'" 
        ...
        <div class="max-w-4xl">
                ...
                <div class="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 p-10">
                    <form action="{{ route('settings.profile.update') }}" method="POST" enctype="multipart/form-data">
                        ...
                        <div class="flex items-center gap-8 mb-10 pb-10 border-b border-slate-50">
                            ...
                            <div>
                                <h3 class="text-xl font-black text-slate-800">{{ __('messages.profile_picture') }}</h3>
                                <p class="text-sm text-slate-400 font-bold mb-2">PNG, JPG or GIF. Max 2MB.</p>
                                <button type="button" @click="document.getElementById('avatar_input').click()" class="px-6 py-2 bg-slate-50 text-slate-600 font-black text-xs rounded-xl hover:bg-emerald-50 hover:text-emerald-600 transition-all">{{ __('messages.upload_new') }}</button>
                            </div>
                        </div>
                        ...
                        <div class="space-y-8">
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-8">
                                ...
                                <div class="space-y-3">
                                    ...
                                    <input type="email" name="email" value="{{ old('email', auth()->user()->email) }}" class="w-full bg-slate-50 border-2 border-transparent focus:border-emerald-500 focus:bg-white rounded-2xl py-4 px-6 font-black text-slate-700 transition-all outline-none">
                            ...
                            <div class="space-y-3">
                                ...
                                <input type="text" name="specialty" value="{{ old('specialty', auth()->user()->specialty ?? __('messages.general_practitioner')) }}" class="w-full bg-slate-50 border-2 border-transparent focus:border-emerald-500 focus:bg-white rounded-2xl py-4 px-6 font-black text-slate-700 transition-all outline-none">
                        ...
                        <div class="space-y-8">
                            ...
                            <div class="space-y-3">
                                ...
                                <div class="grid grid-cols-2 gap-4">
                                    <div class="relative">
                                        <span class="absolute start-4 top-1/2 -translate-y-1/2 text-[10px] font-black text-slate-300">{{ __('messages.from') }}</span>
                                        <input type="time" name="working_hours_from" value="{{ $clinicSettings['working_hours_from'] ?? '09:00' }}" class="w-full bg-slate-50 border-2 border-transparent focus:border-emerald-500 focus:bg-white rounded-2xl py-4 ps-14 pe-6 font-black text-slate-700 transition-all outline-none">
                                    </div>
                                    <div class="relative">
                                        <span class="absolute start-4 top-1/2 -translate-y-1/2 text-[10px] font-black text-slate-300">{{ __('messages.to') }}</span>
                                        <input type="time" name="working_hours_to" value="{{ $clinicSettings['working_hours_to'] ?? '18:00' }}" class="w-full bg-slate-50 border-2 border-transparent focus:border-emerald-500 focus:bg-white rounded-2xl py-4 ps-12 pe-6 font-black text-slate-700 transition-all outline-none">
                                    </div>
                                </div>
                            </div>
                            <div class="pt-4">
            ...
            <div x-show="activeTab === 'security'" x-cloak x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4">
                <div class="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 p-10">
                    <div class="mb-10">
                        <h3 class="text-xl font-black text-slate-800">{{ __('messages.security') }}</h3>
                        <p class="text-sm text-slate-400 font-bold">{{ __('messages.update_password') }}</p>
                    </div>
            ...
            <div x-show="activeTab === 'preferences'" x-cloak x-transition:enter="transition ease-out duration-300" x-transition:enter-start="opacity-0 translate-y-4">
                <div class="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 p-10">
                    <div class="space-y-12">
                        ...
                        <div class="space-y-6">
                            <div>
                                <h3 class="text-xl font-black text-slate-800">{{ __('messages.interface_language') }}</h3>
                                <p class="text-sm text-slate-400 font-bold">{{ __('messages.select_category') }}</p>
                            </div>
                            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                                <a href="{{ route('lang.switch', 'en') }}" class="flex items-center justify-between p-4 rounded-2xl border-2 {{ app()->getLocale() == 'en' ? 'border-emerald-500 bg-emerald-50 text-emerald-600' : 'border-slate-50 bg-slate-50 text-slate-400' }} font-black text-sm transition-all">
                                    English (US)
                                    @if(app()->getLocale() == 'en')
                        ...
                        <div class="flex items-center justify-between py-8 border-t border-slate-50">
                            <div>
                                <h3 class="text-xl font-black text-slate-800">{{ __('messages.dark_mode') }}</h3>
                                <p class="text-sm text-slate-400 font-bold">{{ __('messages.resource_distribution') }}</p>
                            </div>
                            <div class="relative inline-block w-16 h-8 align-middle select-none transition duration-200 ease-in">
                                <input type="checkbox" name="toggle" id="toggle" 
                                       class="toggle-checkbox absolute block w-8 h-8 rounded-full bg-white border-4 border-slate-200 appearance-none cursor-pointer outline-none transition-all duration-300 checked:translate-x-8 checked:border-emerald-500"
                                       :checked="theme === 'dark'"
                                       @change="theme = $el.checked ? 'dark' : 'light'; localStorage.setItem('theme', theme); window.dispatchEvent(new CustomEvent('theme-changed', { detail: theme }))"/>
                                <label for="toggle" class="toggle-label block overflow-hidden h-8 rounded-full bg-slate-100 cursor-pointer transition-all duration-300" :class="theme === 'dark' ? 'bg-emerald-500' : 'bg-slate-100'"></label>
                            </div>
                        </div>
                    </div>
                </div>
